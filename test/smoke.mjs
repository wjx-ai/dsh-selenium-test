/**
 * 冒烟测试：验证插件模块形状 + Selenium 脚本可通过解释器运行并返回合法 JSON。
 *
 * 用法: node test/smoke.mjs
 * 覆盖：
 *  - lib/index.js 能作为 ESM 被 import，且导出 { inject, apply }。
 *  - lib/selenium_test.py 可通过 python 运行（stdin 为空时返回用法错误 JSON，不触网、不需要 selenium）。
 * 该脚本不访问网络，快速且确定。
 */
...
// 2) Python 脚本可执行（空 stdin -> 输出用法错误 JSON，不触网、不需要 selenium）
const proc = spawnSync(python, [script], { encoding: 'utf8', input: '', timeout: 15000 });
if (proc.error) {
  console.error('[smoke] spawn python failed:', proc.error.message);
  console.error('       提示: 请确保 Python 3 在 PATH，或用环境变量 DSH_SELENIUM_TEST_PYTHON 指向解释器。');
  process.exit(1);
}
